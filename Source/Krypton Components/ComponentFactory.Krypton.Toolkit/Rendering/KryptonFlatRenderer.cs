...
{
    internal class KryptonFlatRenderer : KryptonProfessionalRenderer
    {
        ...
        private static readonly int _gripMove = 2;
        private static readonly int _gripLines = 5;
        ...
        private static readonly float _cutItemMenu = 0f;
        ...
        public KryptonFlatRenderer(KryptonColorTable kct)
            : base(kct)
        {
        }
        #endregion
        ...
        protected override void OnRenderButtonBackground(ToolStripItemRenderEventArgs e)
        {
            ...
            RenderToolButtonBackground(e.Graphics, e.Item);
        }
        #endregion
        ...
        protected override void OnRenderItemCheck(ToolStripItemImageRenderEventArgs e)
        {
            ...
            using (AntiAlias aa = new AntiAlias(e.Graphics))
            {
                ...
                using (GraphicsPath borderPath = CreateBorderPath(checkBox, _cutItemMenu))
                {
                    ...
                    using (SolidBrush fillBrush = new SolidBrush(KCT.ToolStripGradientEnd))
                        ...
                        switch (checkState)
                        {
                            case CheckState.Checked:
                                ...
                                using (GraphicsPath tickPath = CreateTickPath(checkBox))
                                {
                                    ...
                                    using (Pen tickPen = new Pen(KCT.ToolStripGradientEnd))
                            ...
                            case CheckState.Indeterminate:
                                ...
                                {
                                    // Draw the tick with a thickish brush
                                    using (SolidBrush tickBrush = new SolidBrush(KCT.ToolStripGradientEnd))
        ...
        protected override void OnRenderMenuItemBackground(ToolStripItemRenderEventArgs e)
        {
                ...
                (e.ToolStrip is ToolStripDropDownMenu))
            {
                if ((e.Item.Pressed) && (e.ToolStrip is MenuStrip))
                {
                    ...
                    using (Brush brush = new SolidBrush(KCT.ToolStripGradientBegin))
                    {
        ...
        protected override void OnRenderStatusStripSizingGrip(ToolStripRenderEventArgs e)
        {
                              ...
                              lightBrush = new SolidBrush(KCT.GripLight))
            {
                ...
                bool rightToLeftOrientation = (e.ToolStrip.RightToLeft == RightToLeft.Yes);
                ...
                for (int i = _gripLines; i >= 1; i--)
                {
                    ...
                    int x = (rightToLeftOrientation ? e.AffectedBounds.Left + 1 : e.AffectedBounds.Right - _gripSize * 2);
                    ...
                    for (int j = 0; j < i; j++)
                    {
                        ...
                        x -= (rightToLeftOrientation ? -_gripMove : _gripMove);
                    }
        ...
        protected override void OnRenderToolStripBackground(ToolStripRenderEventArgs e)
        {
            ...
            else if (e.ToolStrip is StatusStrip)
            {
                ...
                if ((backRect.Width > 0) && (backRect.Height > 0))
                {
                    ...
                    using (Brush backgroundBrush = new SolidBrush(KCT.StatusStripGradientBegin))
                    {
            ...
            else
            {
                ...
                if (e.ToolStrip is MenuStrip)
                {
                    ...
                    using (Brush backgroundBrush = new SolidBrush(KCT.MenuStripGradientBegin))
                    {
                        e.Graphics.FillRectangle(backgroundBrush, e.AffectedBounds);
                    }
                    //base.OnRenderToolStripBackground(e);
                }
                // Заливка фона панели инструментов
                else if (e.ToolStrip is ToolStrip)
                {
                    if (e.ToolStrip.Font != KCT.MenuStripFont)
                        e.ToolStrip.Font = KCT.MenuStripFont;
                                        
                    using (Brush backgroundBrush = new SolidBrush(KCT.ToolStripGradientBegin))
                    {
                        e.Graphics.FillRectangle(backgroundBrush, e.AffectedBounds);
                    }
                }
                else
                {
                    ...
                    if ((backRect.Width > 0) && (backRect.Height > 0))
                    {
                        ...
                        {
                            using (Brush backBrush = new SolidBrush(KCT.ToolStripGradientBegin))
                            {
        ...
        protected override void OnRenderToolStripBorder(ToolStripRenderEventArgs e)
        {
            ...
            {
                // В этой теме не рисуем границы строки состояния
            }
            // Граница панели инструментов
            else if (e.ToolStrip is ToolStrip && !(e.ToolStrip is MenuStrip))
            {
                using (Pen darkBorder = new Pen(KCT.ToolStripBorder),
                           lightBorder = new Pen(KCT.SeparatorLight))
                {
                    //Rectangle toolStripBounds = new Rectangle(e.AffectedBounds.Left, e.AffectedBounds.Top, e.AffectedBounds.Width - 1, e.AffectedBounds.Height - 1);
                    Rectangle toolStripBounds = new Rectangle(e.AffectedBounds.Left - 1, e.AffectedBounds.Top - 1, e.AffectedBounds.Width + 1, e.AffectedBounds.Height);
                    e.Graphics.DrawRectangle(darkBorder, toolStripBounds);
                }
            }
            else
            {
        ...
        protected override void OnRenderGrip(ToolStripGripRenderEventArgs e)
        {
            ...
            Rectangle gripRectangle = new Rectangle(e.GripBounds.Location.X, e.GripBounds.Location.Y + 1, e.GripBounds.Size.Width, e.GripBounds.Size.Height - 3);
            

            //using (Pen pen = new Pen(KCT.GripLight))
            using (Brush brush = new SolidBrush(KCT.MenuStripGradientBegin))
            {
                if (e.GripDisplayStyle == ToolStripGripDisplayStyle.Vertical)
                {
                    g.FillRectangle(brush, gripRectangle);
                    //g.DrawLine(pen, e.GripBounds.X, e.GripBounds.Y + 4, e.GripBounds.X, e.GripBounds.Bottom - 5);
                    //g.DrawLine(pen, e.GripBounds.X + 2, e.GripBounds.Y + 4, e.GripBounds.X + 2, e.GripBounds.Bottom - 5);
                }
                else
                {
                    g.FillRectangle(brush, gripRectangle);
                    //g.DrawLine(pen, e.GripBounds.X + 4, e.GripBounds.Y, e.GripBounds.Right - 5, e.GripBounds.Y);
                    //g.DrawLine(pen, e.GripBounds.X + 4, e.GripBounds.Y + 2, e.GripBounds.Right - 5, e.GripBounds.Y + 2);
                }
            }
        }
        #endregion
        ...
        private void RenderToolButtonBackground(Graphics g, ToolStripItem toolStripItem)
        {
            if (toolStripItem.Enabled)
            {
                // Ensure we have cached the objects we need
                UpdateCache();
                ...
                if (toolStripItem is ToolStripButton)
                {
                    ...
                    if (button != null && button.Checked)
                    {
                        if (button.Pressed)
                        {
                            using (Brush brush = new SolidBrush(KCT.ToolStripGradientMiddle))
                            {
                                g.FillRectangle(brush, new Rectangle(Point.Empty, button.Bounds.Size));
                            }
                        }
                        ...
                        else
                        {
                            ...
                            using (Brush brush = new SolidBrush(KCT.ToolStripBorder))
                            {
                                g.FillRectangle(brush, new Rectangle(Point.Empty, button.Bounds.Size));
                            }
                        }
                ...
                if (toolStripItem.Pressed)
                {
                    using (Brush brush = new SolidBrush(KCT.ToolStripBorder))
                    {
                        g.FillRectangle(brush, new Rectangle(Point.Empty, toolStripItem.Bounds.Size));
                    }
                }
        ...
        private void DrawGripGlyph(Graphics g,
                                   ...
                                   Brush lightBrush)
        {
            g.FillRectangle(lightBrush, x + _gripOffset, y + _gripOffset, 1, 1);
        ...
        private void DrawToolStripSeparator(Graphics g, bool vertical, Rectangle rect, Color lightColor, Color darkColor, int horizontalInset, bool rtl)
        {
            if (vertical)
            {
                using (Pen separatorPen = new Pen(lightColor))
                {
                    g.DrawLine(separatorPen, rect.Width / 2, 1, rect.Width / 2, rect.Bottom - 2);
                }
            }
            else
            {
                using ( Pen separatorPen = new Pen(lightColor))
                {
                    g.DrawLine(separatorPen, 1, rect.Height / 2, rect.Right - 2, rect.Height / 2);
                }
            }
        }
